...
export function LateLeaderboardCard({ rows }: { rows: LateLeaderboardRow[] }) {
  return (
    <div className="rounded-xl border border-outline-variant bg-white p-6 shadow-sm">
      <h3 className="mb-4 text-lg font-bold">ท็อป 5 มาสายประจำเดือน</h3>
      {rows.length === 0 ? (
        ...
        <ul className="space-y-3">
          {rows.map((row, i) => (
            <li key={row.employeeId}>
              ...
              >
                <span className="w-6 text-center text-lg">{MEDALS[i] ?? <span className="text-sm font-bold text-on-surface-variant">{i + 1}</span>}</span>
