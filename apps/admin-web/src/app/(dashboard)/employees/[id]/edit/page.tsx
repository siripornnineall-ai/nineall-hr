...
export default async function EditEmployeePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: employee }, departments, positions, managers, { data: branches }, { data: compensation }] = await Promise.all([
    supabase
      .from("employees")
      .select(
        "id, employee_code, first_name, last_name, nickname, phone, personal_email, hire_date, employment_type, branch_id, department_id, job_position_id, manager_employee_id"
      )
