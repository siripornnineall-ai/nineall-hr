...
interface NotificationRow {
  ...
  is_read: boolean;
  created_at: string;
  data: { leave_request_id?: string } | null;
}

// Where clicking a notification should take you — keyed by notifications.type.
...
export function NotificationBell() {
  ...
  useEffect(() => {
    load();
    // A lightweight poll rather than a realtime subscription — this only needs to feel
    // reasonably fresh, not instant.
    ...
    <div ref={containerRef} className="relative flex items-center gap-3">
      {pushStatus === "unsubscribed" && (
        <button
          ...
          title="เปิดการแจ้งเตือนบนมือถือ/หน้าจอ"
          aria-label="เปิดการแจ้งเตือนบนมือถือ/หน้าจอ"
          className="relative text-on-surface-variant transition-colors hover:text-primary disabled:opacity-60"
        >
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-2 w-80 rounded-xl border border-outline-variant bg-white shadow-lg">
          ...
          {pushStatus === "denied" && (
            <p className="border-b border-outline-variant bg-status-danger/10 px-4 py-2.5 text-xs text-status-danger">
              การแจ้งเตือนถูกปิดไว้ในเบราว์เซอร์ — ไปที่การตั้งค่าเบราว์เซอร์/มือถือแล้วอนุญาตการแจ้งเตือนให้เว็บนี้ ถ้าอยากได้รับแจ้งเตือนขึ้นหน้าจอ
            </p>
          )}
